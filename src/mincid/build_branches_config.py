...
""" % (self.__config['vcs']['authcmd'], self.__config['vcs']['url'],
       self.__config['dest']), "UTF-8"))
        ...
        for branch_name in self.__config['branches']:
            ...
            p.stdin.write(bytes(
"""su - builder --command 'cd %s && git checkout %s && mkdir -p /artifacts/%s && cp mincid.json /artifacts/%s'
""" %(self.__config['dest'], branch_name, name_for_fs, name_for_fs), 'UTF-8'))
...
#            with open(stdouterr_filename, "w") as fd_stdouterr:
#                p = subprocess.Popen(["sbatch", "--job-name=Project_%s" % self.__name,
#                                      "--export=PYTHONPATH",
